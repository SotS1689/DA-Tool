...
2. Paste your passage text into the Propositions panel and use **Insert Propositions** to split it into rows.
3. Select one or more propositions, then use **Add Two-Node Bracket** or **Add Single-Node Bracket** to connect them.
4. Right-click a bracket's corner to label it with a logical relationship.
5. Drag rows in the sidebar to reorder propositions, use Tab/Shift+Tab to indent/outdent, and Delete to remove a selected item.
...
See the source in [`obsidian-plugin/src/`](obsidian-plugin/src/) — `main.ts` registers the `.da` view and commands, `DAView.ts` contains the diagramming engine.
