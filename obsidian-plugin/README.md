...
See the source in [`src/`](src/) — `main.ts` registers the `.da` view, commands, and settings, `DAView.ts` contains the diagramming engine and flow canvases. The web app is the single self-contained [`index.html`](../index.html).
